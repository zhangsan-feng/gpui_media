...
const BASE_URL: &str = "https://haohuazy.com";
const AUTHOR: &str = "haohuazy";

pub struct HaohuazyInterface;

impl NetworkStaticInterface for HaohuazyInterface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    ...
                    .await
                    .expect("request haohuazy play page error");
                let html = response
                    .text()
                    .await
                    .expect("haohuazy play page html parse error")
                    .replace("\\/", "/");
                ...
                Regex::new(r#"https?://[^\s"'<>]+\.m3u8[^\s"'<>]*"#)
                    ...
                    .expect("haohuazy play url not found")
            })
        })
    }
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(&abs_url(&params.source), headers())
                    .await
                    .expect("request haohuazy detail page error");
                let html = response
                    .text()
                    .await
                    .expect("haohuazy detail html parse error");
...
pub async fn search(keyword: String) -> Vec<NetworkStatic> {
    ...
    let response = match HttpClient::new().get_for_html(&url, headers()).await {
        ...
        Err(err) => {
            log::info!("request haohuazy search error: {}", err);
            return Vec::new();
        }
    };
    ...
    match response.text().await {
        ...
        Err(err) => {
            log::info!("read haohuazy search html error: {}", err);
            Vec::new()
        }
    }
}
...
fn search_url(keyword: &str) -> String {
    let mut url = Url::parse(&format!("{}/index.php/vod/search.html", BASE_URL))
        .expect("invalid haohuazy search url");
...
fn parse_search_html(html: &str) -> Vec<NetworkStatic> {
    ...
    for capture in link_re.captures_iter(html) {
        ...
        videos.push(NetworkStatic {
            ...
            source,
            func: Arc::new(HaohuazyInterface),
        });
    }
