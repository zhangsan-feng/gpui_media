...
const BASE_URL: &str = "https://ukuzy0.com";
const AUTHOR: &str = "ukuzy0";

pub struct Ukuzy0Interface;

impl NetworkStaticInterface for Ukuzy0Interface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    ...
                    .await
                    .expect("request ukuzy0 play page error");
                let html = response
                    .text()
                    .await
                    .expect("ukuzy0 play page html parse error")
                    .replace("\\/", "/");
                ...
                Regex::new(r#"https?://[^\s"'<>]+\.m3u8[^\s"'<>]*"#)
                    ...
                    .expect("ukuzy0 play url not found")
            })
        })
    }
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(&abs_url(&params.source), headers())
                    .await
                    .expect("request ukuzy0 detail page error");
                let html = response
                    .text()
                    .await
                    .expect("ukuzy0 detail html parse error");
...
pub async fn search(keyword: String) -> Vec<NetworkStatic> {
    ...
    let response = match HttpClient::new().get_for_html(&url, headers()).await {
        ...
        Err(err) => {
            log::info!("request ukuzy0 search error: {}", err);
            return Vec::new();
        }
    };
    ...
    match response.text().await {
        ...
        Err(err) => {
            log::info!("read ukuzy0 search html error: {}", err);
            Vec::new()
        }
    }
}
...
fn search_url(keyword: &str) -> String {
    let mut url = Url::parse(&format!("{}/index.php/vod/search.html", BASE_URL))
        .expect("invalid ukuzy0 search url");
