...
const BASE_URL: &str = "https://jszy333.com";
const AUTHOR: &str = "jszy333";

pub struct Jszy333Interface;

impl NetworkStaticInterface for Jszy333Interface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    ...
                    .await
                    .expect("request jszy333 play page error");
                let html = response
                    .text()
                    .await
                    .expect("jszy333 play page html parse error")
                    .replace("\\/", "/");
                ...
                Regex::new(r#"https?://[^\s"'<>]+\.m3u8[^\s"'<>]*"#)
                    ...
                    .expect("jszy333 play url not found")
            })
        })
    }
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(&abs_url(&params.source), headers())
                    .await
                    .expect("request jszy333 detail page error");
                let html = response
                    .text()
                    .await
                    .expect("jszy333 detail html parse error");
...
pub async fn search(keyword: String) -> Vec<NetworkStatic> {
    ...
    let response = match HttpClient::new().get_for_html(&url, headers()).await {
        ...
        Err(err) => {
            log::info!("request jszy333 search error: {}", err);
            return Vec::new();
        }
    };
    ...
    match response.text().await {
        ...
        Err(err) => {
            log::info!("read jszy333 search html error: {}", err);
            Vec::new()
        }
    }
}
...
fn search_url(keyword: &str) -> String {
    let mut url = Url::parse(&format!("{}/index.php/vod/search.html", BASE_URL))
        .expect("invalid jszy333 search url");
