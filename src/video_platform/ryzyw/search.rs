...
const BASE_URL: &str = "https://www.ryzyw.com";
const AUTHOR: &str = "ryzyw";

pub struct RyzywInterface;

impl NetworkStaticInterface for RyzywInterface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    ...
                    .await
                    .expect("request ryzyw play page error");
                let html = response
                    .text()
                    .await
                    .expect("ryzyw play page html parse error")
                    .replace("\\/", "/");
                ...
                Regex::new(r#"https?://[^\s"'<>]+\.m3u8[^\s"'<>]*"#)
                    ...
                    .expect("ryzyw play url not found")
            })
        })
    }
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(&abs_url(&params.source), headers())
                    .await
                    .expect("request ryzyw detail page error");
                let html = response
                    .text()
                    .await
                    .expect("ryzyw detail html parse error");
...
pub async fn search(keyword: String) -> Vec<NetworkStatic> {
    ...
    let response = match HttpClient::new().get_for_html(&url, headers()).await {
        ...
        Err(err) => {
            log::info!("request ryzyw search error: {}", err);
            return Vec::new();
        }
    };
    ...
    match response.text().await {
        ...
        Err(err) => {
            log::info!("read ryzyw search html error: {}", err);
            Vec::new()
        }
    }
}
...
fn search_url(keyword: &str) -> String {
    let mut url = Url::parse(&format!("{}/index.php/vod/search.html", BASE_URL))
        .expect("invalid ryzyw search url");
