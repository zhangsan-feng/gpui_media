...
const BASE_URL: &str = "https://ffzy5.tv";
const AUTHOR: &str = "ffzy5";

pub struct Ffzy5Interface;

impl NetworkStaticInterface for Ffzy5Interface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    ...
                    .await
                    .expect("request ffzy5 play page error");
                let html = response
                    .text()
                    .await
                    .expect("ffzy5 play page html parse error")
                    .replace("\\/", "/");
                ...
                Regex::new(r#"https?://[^\s"'<>]+\.m3u8[^\s"'<>]*"#)
                    ...
                    .expect("ffzy5 play url not found")
            })
        })
    }
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(&abs_url(&params.source), headers())
                    .await
                    .expect("request ffzy5 detail page error");
                let html = response
                    .text()
                    .await
                    .expect("ffzy5 detail html parse error");
...
pub async fn search(keyword: String) -> Vec<NetworkStatic> {
    ...
    let response = match HttpClient::new().get_for_html(&url, headers()).await {
        ...
        Err(err) => {
            log::info!("request ffzy5 search error: {}", err);
            return Vec::new();
        }
    };
    ...
    match response.text().await {
        ...
        Err(err) => {
            log::info!("read ffzy5 search html error: {}", err);
            Vec::new()
        }
    }
}
...
fn search_url(keyword: &str) -> String {
    let mut url = Url::parse(&format!("{}/index.php/vod/search.html", BASE_URL))
        .expect("invalid ffzy5 search url");
