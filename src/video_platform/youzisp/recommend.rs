...
impl NetworkStaticInterface for YouziVipInterface {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(
                        format!("https://youzisp.tv{}", params.source).as_str(),
                        headers(),
                    )
                    .await
                    .expect("r");
    ...
    fn detail(&self, params: &NetworkStatic) -> Vec<NetworkStatic> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let response = HttpClient::new()
                    .get_for_html(params.source.as_str(), headers())
                    .await
                    .expect("");
                let response_text = response.text().await.expect("html parse error");
...
async fn request_web() -> Vec<NetworkStatic> {
    ...
    let response = HttpClient::new()
        .get_for_html("https://youzisp.tv", headers())
        .await
    ...
    for element in document
        .select(&Selector::parse(".main > .content > .module").expect(""))
        .by_ref()
    {
        ...
        for elem in element
            .select(&Selector::parse(".module-main").expect(""))
            .by_ref()
        {
            for ele in elem
                .select(&Selector::parse(".module-items > a").expect(""))
                .by_ref()
            {
                let name = ele
                    .value()
